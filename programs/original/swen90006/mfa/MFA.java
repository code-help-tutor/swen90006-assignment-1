 ...
 * authentication). 
 * 
 * For admin staff there is a three factor authentication where a staff
 * member can register face. The system will check if the face matches (along
 * with correct password and registered device...it will let it in
 * otherwise, will unauthorise. #make it three factor
 *
 ...
 * or
 *
 * - They are authenticated to authentication status DOUBLE, have
 *   registered a device, and have validated they have access to the
...
public class MFA{
	...
	public enum AuthenticationStatus{NONE, SINGLE, DOUBLE, TRIPLE};
	...
	// for facerecognisation records
	private Map<String, String> faceRecognised;// added by Najam
	...
	public MFA(){
		...
		faceRecognised = new HashMap<String, String>();// added by name to accommodate face recognition.
	 ...
	 */
	public void register(String username, String password, String deviceID, String faceid)// faceid added by Najam
	...
	public AuthenticationStatus login(String username, String password)
		...
		if (checkUsernamePassword(username, password)) {
			...
			AuthenticationStatus twoFactor= AuthenticationStatus.SINGLE;
	...
	public AuthenticationStatus login(String username, String password, String deviceId, String faceId)
		...
		if (checkUsernamePassword(username, password)) {
			...
			if (deviceIDs.get(username) != null) {
				sendPushNotification(username, deviceIDs.get(username));
				twoFactor =respondToPushNotification(username, deviceIDs.get(username));
				authenticationStatus.put(username, twoFactor);
			}
	...
	/**
	 * 
	 * Simulating that a third factor is added such as facial recognition
	 *
	 * @param username
	 * @param deviceID
	 * @param faceMatch
	 ...
	 * @throws FaceMismatchException
	 **/
	public AuthenticationStatus faceRegonised(String username, String deviceID, String facialId)
			throws NoSuchUserException, IncorrectDeviceIDException, FaceMismatchException{
		if (!isUser(username)) {
			throw new NoSuchUserException(username);
		}else if (authenticationStatus.get(username) == AuthenticationStatus.SINGLE) {
			if (deviceIDs.get(username) != null && deviceIDs.get(username) != deviceID) {
		...
		if(authenticationStatus.get(username) == AuthenticationStatus.DOUBLE
				&& faceRecognised.containsKey(username)
